...
inputPort ClientPort {
  Location: "socket://localhost:9004" // #dynamic
	Protocol: http {
		.format -> format;
...
inputPort AdminPort {
  Location: "socket://localhost:10004" // #dynamic
	Protocol: http {
		.format -> format;
...
define setElectionTimer {
  ...
  electionTimerReq.port = "8004"; // #dynamic
  ...
  [ClientRequest(request)(response) {
    if(!global.status.leader)
      if(is_defined(global.status.leaderAddress))
        ...
        response.address = "socket://localhost:9005" // #dynamic
  ...
  [AdminRequest(request)(response) {
    if(!global.status.leader)
      if(is_defined(global.status.leaderAddress))
        ...
        response.address = "socket://localhost:10005" // #dynamic
