...
inputPort ClientPort {
  Location: "socket://localhost:9001" // #dynamic
	Protocol: http {
		.format -> format;
...
inputPort AdminPort {
  Location: "socket://localhost:10001" // #dynamic
	Protocol: http {
		.format -> format;
...
define setElectionTimer {
  ...
  electionTimerReq.port = "8001"; // #dynamic
      ...
      if(ackRequest.term > global.status.currentTerm) {
        ...
        if(ackRequest.success) {
          ...
          scope( sendAppendEntries ) {
            if(int(csets.serverSID) == 2) { // #dynamic
              println@Console( "A replied FALSE resend appendentries" )();
              appendEntriesA
            } else if(int(csets.serverSID) == 3) { // #dynamic
  ...
  [ClientRequest(request)(response) {
    if(!global.status.leader)
      if(is_defined(global.status.leaderAddress))
        ...
        response.address = "socket://localhost:9002" // #dynamic
  ...
  [AdminRequest(request)(response) {
    if(!global.status.leader)
      if(is_defined(global.status.leaderAddress))
        ...
        response.address = "socket://localhost:10002" // #dynamic
