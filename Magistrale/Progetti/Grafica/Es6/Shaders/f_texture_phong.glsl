// Fragment shader: Texture-Phong shading
// ================
#version 320 es
...
void main() {
    ...
    vec3 ambient = light.power * vec3(0);

    // Diffuse component: k_d * (l * n) * I_l
    vec3 diffuse = vec3(0) * (max(dot(normal, lightSourceDirection), 0.0)) * light.power * light.color;
    ...
    fragColor = texture(textureBuffer, _TexCoord).xyz + ambient + diffuse + specular;
}